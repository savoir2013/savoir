...
import ca.gc.nrc.iit.savoir.scheduler.types.ImpossibleReservation;
 ...
 * 
 */
public class MaxConcurrentUsers implements RequirementChecker {
	 ...
	 * maximum number of users at a time
	 */
	@Override
	public CheckResponse check(Session s, long offset, Resource r, Constraint c) {
		...
		// integrity check:
		if (countOccurences(s, r) > max) {
			response.setSuccessful(false);
			response.setTimeOffset(0);
			SchedulingConflict[] sc = { new ImpossibleReservation("Resource "
					+ r.getResourceID() + " (" + r.getDescription()
					+ ") cannot be used by more than " + max + " users at a time.") };
