...
import ca.gc.nrc.iit.oauth.common.signature.pem.PEMReader;
import ca.gc.nrc.iit.oauth.common.signature.pem.PKCS1EncodedKeySpec;
 ...
 * 
 * OAuthToken t = new OAuthToken(consumer_key, consumer_privateRSAKey, token, "");
 *
 * consumer_privateRSAKey must be an RSA signing key and
 * of type String. The key must either PKCS#1 or PKCS#8 encoded.
 * 
 ...
 * to know the consumer's public key. You set it up
 * (on the service provider's side) like this:
 * 
 * OAuthToken t = new OAuthToken(consumer_key, consumer_publicRSAKey, token, "");
 *
 * consumer_publicRSAKey must be the consumer's public RSAkey and
 * of type String, X509-encoded and then Base64-encoded.
 *
 * @author Dirk Balfanz, Google
 * @author Aaron Moss, NRC-IIT
 */
public class RsaSha1SignatureMethod extends OAuthSignatureMethod {

	public static final String NAME = RSA_SHA1;
    ...
    protected String getSignature(String baseString, OAuthToken token, OAuthConsumer consumer) 
        ...
        try {
            if (privateKey == null || !token.equals(this.token)) {
            	privateKey = loadPrivateKey(consumer.getConsumerSecret());
            }
    ...
    protected boolean isValid(String signature, String baseString, OAuthToken token, 
        ...
        try {
        	if (publicKey == null || !token.equals(this.token)) {
            	publicKey = loadPublicKey(consumer.getConsumerSecret(), false);
            }
