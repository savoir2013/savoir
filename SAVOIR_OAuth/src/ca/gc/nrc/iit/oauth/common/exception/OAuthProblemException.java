...
public class OAuthProblemException extends OAuthException {
    ...
    public String getMessage() {
        ...
        if (msg != null)
            return msg;
        Object response = getParameters().get(OAuth.HTTP_RESPONSE);
        if (response != null) {
        ...
        if (response != null) {
            return OAuth.HTTP_STATUS_CODE + " " + response;
        }
        return null;
    }
    ...
    public List<Parameter> getParameterList() {
    	List<Parameter> l = new ArrayList<Parameter>(parameters.size());
    	for (Map.Entry<String, Object> e : parameters.entrySet()) {
    		l.add(new Parameter(e.getKey(), OAuth.toString(e.getValue())));
    	}
    ...
    public int getHttpStatusCode() {
        Object code = getParameters().get(OAuth.HTTP_STATUS_CODE);
        if (code == null) {
            return 200;
