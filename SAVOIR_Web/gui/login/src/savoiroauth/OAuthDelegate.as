...
package savoiroauth
{
	...
	public class OAuthDelegate extends EventDispatcher
	{
		...
		public var isAuthRequestTokenDelegate:Boolean = false;
...
//			}
	...
	//			urlLoader.addEventListener(HTTPStatusEvent["HTTP_RESPONSE_STATUS"], httpStatusHandler);
		...
		{
			if(isAuthRequestTokenDelegate == true)
			{
				//Alert.show("Get Auth Req Token Response");
				dispatchEvent( new AuthEvent(AuthEvent.AUTH_REQUEST_TOKEN) );
			}else{
...
//						data = JSON.decode(rawData as String);
		...
		protected function httpStatusHandler(event:HTTPStatusEvent):void
		{
			...
			dispatchEvent( new FaultEvent(FaultEvent.FAULT, msg, event.status) );
		}
